...
- **Heading system:** Single source of truth in `components/common/section-heading.tsx`. `SectionHeading` accepts `variant` prop: `page` (default — h1, text-headline, font-bold, with optional subtitle/action), `landing` (h2, text-headline→headline-md responsive, font-black, overline slot, split layout option), `subsection` (h3, text-body-lg, font-semibold, for settings groups). Override tag via `as` prop. Old components `PageHeading`, `SectionHeader`, `SectionTitle`, and landing `SectionHeading` are deleted — never recreate them.
